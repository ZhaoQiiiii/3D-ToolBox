...
/**
 * Load and preprocess a scene graph. The returned `source` mirrors the
 * backend's X-Scene-Source response header ("saved"|"exported") and tells the
 * caller which copy was actually served, so a subsequent export can use the
 * same base instead of silently rebuilding exported/ from saved/.
 */
export async function loadSceneGraph(
  path: string,
): Promise<{ data: SceneData; source: "saved" | "exported" }> {
  const resp = await fetch(path, { cache: "no-store" });
  if (!resp.ok) throw new Error(`Failed to load ${path}: ${resp.status}`);
  const source: "saved" | "exported" =
    resp.headers.get("x-scene-source") === "exported" ? "exported" : "saved";
  ...
  return { data: { areas, polys, topoNodes, topoEdges, objects }, source };
}
