...
export function ObjectsLayer({
  ...
  // area alone would force one material per area and paint every object with
  // the first object's color, discarding per-object colors.
  const groups = useMemo(() => {
    const byAreaColor = new Map<string, { areaId: number; color: string; objs: SceneObject[] }>();
    for (const o of objects) {
      const key = `${o.areaId}|${o.colorHex}`;
      let g = byAreaColor.get(key);
      if (!g) {
        g = { areaId: o.areaId, color: o.colorHex, objs: [] };
        byAreaColor.set(key, g);
      }
      g.objs.push(o);
    }
    return [...byAreaColor.values()].map(({ areaId, color, objs }) => {
      ...
      return { areaId, positions: pos, color, dimmed };
    });
